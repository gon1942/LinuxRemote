...
hamonizeServerSettings() {
    echo "3-------------------$CENTER_BASE_URL" >>$LOGFILE
...
hamonizeHelpSettings() {
    ...
    echo " 1. Hamonize Help Application Install ================ [start]" >>$LOGFILE
    sudo apt-get install hamonize-help -y 2>>$LOGFILE # >/dev/null
    echo "" >>$LOGFILE
    ...
    if [ "" = "$PKG_OK" ]; then
        ...
        echo " Hamonize Help Application Install Fail.!!" >>$LOGFILE
        echo "" >>$LOGFILE
        echo "##################################################################" >>$LOGFILE
        ...
        echo " Hamonize Help Application Install === [end]" >>$LOGFILE
        return 0
    fi
...
remoteToolSettings() {
    ...
    echo " 8.  Hamonize Remote Tool install ============== [start]" >>$LOGFILE
    ...
    OSGUBUN=$(lsb_release -i | awk -F : '{print toupper($2)}' | tr -d '\t')
    ...
    echo " 8.  Hamonize Remote Tool install ============== [end]" >>$LOGFILE
    sleep 1

    # echo " 8.  hamonize-user set auth key  ============== [start]" >>$LOGFILE
    ...
    if [ "" = "$PKG_OK" ]; then
        ...
        sudo ufw allow 11100 >/dev/null
        sudo ufw allow 11400 >/dev/null
        return 0
    fi
...
ldapSettings() {
    ...
    if [ "$Ldap_used" == "Y" ]; then
        echo " 8.  Hamonize Ldap Settings &install ============== [start]" >>$LOGFILE
...
timeshiftSettings() {
    ...
    if [ "" = "$PKG_OK" ]; then
        retval=1
        echo "timeshift install Fail >  Error Check LogFile Location is /tmp/hm_timeshift.error " >>$LOGFILE
...
telegrafSettings() {
    ...
    if [ "" = "$PKG_OK" ]; then
        ...
        echo " 6-1.  telegraf Setting  Fail ============== [end]" >>$LOGFILE
        echo "##################################################################" >>$LOGFILE
...
InstallHamonizeProgram() {
    ...
    if [ "$AUTOLOGIN_USED_YN" == "Y" ]; then
        LoginUserid=$(awk -F'[/:]' '{if ($HOME_USERID == 1000 && $HOME_USERID != 65534) print $1}' /etc/passwd)
    ...
    if [ "$IS_DISPLAY_YN" == "Y" ]; then
        ...
        # echo "shell log ] ---------- ret_hamonizeHelpSettings=========$ret_hamonizeHelpSettings" >>$LOGFILE
