...
char *hamonizeDevice()
{
	...
	int ret = system("/usr/local/bin/center-lockdown");
	WEXITSTATUS(ret);
	printf("ret : %d \n", ret);
	return 0;
}
...
char *hamonizeDeviceSendLog()
{
	...
	int ret = system("/etc/hamonize/agentJobs/usbLogSend");
	WEXITSTATUS(ret);
	printf("ret : %d \n", ret);
	return 0;
}
...
char *hamonizeUfw()
{
	...
	int ret = system("/bin/bash /etc/hamonize/agentJobs/ufwjob");
...
int main(int argc, char *argv[])
{
	...
	while (auparse_next_event(au) > 0)
	{
		...
		// Application Block Rules Policy
